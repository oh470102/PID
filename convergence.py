...
PID_baseline = np.array([[-46, 130], [-25, 126], [-20,  16]])   # baseline, ISE = 737
...
axs[0].plot(np.linspace(start=0, stop=len(x_baseline), num=len(x_baseline)), x_baseline, label='BASELINE (ISE=737)')
...
axs[1].plot(np.linspace(start=0, stop=len(theta_baseline), num=len(theta_baseline)), theta_baseline, label='BASELINE (ISE=737)')
...
# show plot
plt.tight_layout()
plt.savefig('ISE=609 with random resets.png')
plt.show()
